<?php 
    ...
    if(count($cart)>0){
        if($_POST['status']=='dibayar'){
            ...
            mysqli_query($conn,"insert into transaksi (id_member,tgl,batas_waktu,status,dibayar,id_user) value('".$_POST['nama']."','".date('Y-m-d')."','".$tgl_selesai."','baru','".$_POST['status']."','".$_SESSION['id_user']."')");
        }
